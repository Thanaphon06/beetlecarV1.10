Blockly.Python['new_motor3'] = function(block) {
  ...
  return code;
        ...
        Blockly.Python['servo'] = function(block) { //ชื่อที่เราตั้งไว้
          ...
          if (value_pin == '16') {
            code = `PWM(Pin(16), freq=50).duty(int(0.5 * ${value_angle} + 31))\n`
          if(value_angle >= 90){
            code = `PWM(Pin(16), freq=50).duty(int(0.5 * 90 + 31))\n`
            }
          if(value_angle <= 0){
            code = `PWM(Pin(16), freq=50).duty(int(0.5 * 0 + 31))\n`
          }
        }
          else if(value_pin == '17'){
            code = `PWM(Pin(17), freq=50).duty(int((4/9) * ${value_angle} + 48))\n`
          if(value_angle >= 90){
            code = `PWM(Pin(17), freq=50).duty(int((4/9) * 90 + 48))\n`
          }
          if(value_angle <= 0){
            code = `PWM(Pin(17), freq=50).duty(int((4/9) * 0 + 48))\n`
          }
        }
